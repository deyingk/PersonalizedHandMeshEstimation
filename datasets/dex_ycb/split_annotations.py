...
def main():
    ...
    configs = [('s0','train'), ('s0','val'), ('s0','test')]
    ...
    for setup, split in configs: 
        ...
        for i, data in enumerate(dataset):
            ...
            if data['mano_side'] == 'right':
                ...
                if not (np.array(label['joint_3d'])==-1).all():
                    ...
                    this_anno = {'img_path':data['color_file'], 
                                    'joint_2d': label['joint_2d'].squeeze(),
                                    'joint_3d': label['joint_3d'].squeeze(),
                                    'projected_joint_2d': projection_2d,
        ...
        with open(os.path.join(save_dir, setup+'_'+split+'_no_crop.pkl'), 'wb') as f:
