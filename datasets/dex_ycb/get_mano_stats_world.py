...
all_mano_anno = []
for anno in annos:
    all_anno_to_save.append([anno['world_regressed_joint_3d'], anno['world_mesh'], anno['world_mano_params']])
    all_mano_anno.append(anno['world_mano_params'])
...
with open(os.path.join(anno_dir, 'all_world_mano_anno.pkl'), 'wb') as f:
    pickle.dump(all_anno_to_save, f)
with open(os.path.join(anno_dir, 'dex_ycb_world_mano_stats.pkl'), 'wb') as f:
